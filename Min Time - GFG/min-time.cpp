...
class Solution {
  ...
  typedef long long ll;
    ll dp[100001][3];
    // Now choosing the which value to visit first from min,max of each type
    ll helper(ll idx,vector<vector<ll>>& nums,ll val){
        if(idx==nums.size()){
            if(val==1){
                return abs(nums[idx-1][1]);
            }else return abs(nums[idx-1][2]);
        }
        if(dp[idx][val]!=-1) return dp[idx][val];
        ll prev=0;
        // Setting the prev coordinate from where we came
        if(idx==0){
            prev=0;
        }
        else if(val==1){
    ...
    long long minTime(int n, vector<int> &locations, vector<int> &types) {
        ...
        // Making a new array for simplicity
        // Storing the minimum and maximum value of each type
